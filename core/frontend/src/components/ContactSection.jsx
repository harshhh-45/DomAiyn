...
export default function ContactSection() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setStatus({ loading: false, success: false, error: 'Could not reach server. Please check your connection.' });
        }
    };
    ...
    return (
        <section id="Contact" ref={ref} className="relative overflow-hidden flex items-center py-8 sm:py-10 md:py-12 z-10">
            ...
            >
                ...
                {/* Two Column Layout: Form + Offices + Social */}
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 sm:gap-10 md:gap-12">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="name" className="block text-xs sm:text-sm font-space mb-1.5 sm:mb-2 text-white/70">Name</label>
                            ...
                            <div>
                                <label htmlFor="email" className="block text-xs sm:text-sm font-space mb-1.5 sm:mb-2 text-white/70">Email</label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="phone" className="block text-xs sm:text-sm font-space mb-1.5 sm:mb-2 text-white/70">Phone Number</label>
                            ...
                            <div>
                                <label htmlFor="subject" className="block text-xs sm:text-sm font-space mb-1.5 sm:mb-2 text-white/70">Subject</label>
                        ...
                        {/* Message */}
                        <div>
                            <label htmlFor="message" className="block text-xs sm:text-sm font-space mb-1.5 sm:mb-2 text-white/70">Message</label>
                        ...
                        <div>
                            <motion.button type="submit" disabled={status.loading}
                                ...
                                className="w-full py-3 sm:py-3.5 bg-gradient-to-r from-purple-500 via-blue-500 to-pink-500 rounded-lg font-space font-semibold text-sm sm:text-base text-white hover:shadow-2xl hover:shadow-purple-500/30 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                                {status.loading ? (
                            ...
                            {status.success && (
                                <motion.p initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                                    className="mt-3 text-center text-sm text-green-400 font-inter">
                                    ✅ Message sent! We'll get back to you soon.
                                </motion.p>
                            )}
                            {status.error && (
                                <motion.p initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                                    className="mt-3 text-center text-sm text-red-400 font-inter">
                                    ❌ {status.error}
                                </motion.p>
                            )}
                        </div>
                    ...
                    {/* Right Side: Office Locations + Social Links - Takes 2 columns */}
                    <motion.div
                        initial={{ opacity: 0, x: 30 }}
                        animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: 30 }}
                        transition={{ duration: 0.6, delay: 0.4 }}
